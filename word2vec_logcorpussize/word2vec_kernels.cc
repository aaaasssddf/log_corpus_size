...
class SkipgramWord2vecOp : public OpKernel {
 public:
  explicit SkipgramWord2vecOp(OpKernelConstruction* ctx)
    ...
    OP_REQUIRES_OK(ctx, ctx->GetAttr("vocab_size_in", &vocab_size_in));
 ...
 private:
  ...
  Status Init(Env* env, const string& filename) {
    ...
    std::vector<WordFreq> tmp_ordered;
    for (const auto& p : word_freq) {
      tmp_ordered.push_back(p);
    }
    std::sort(tmp_ordered.begin(), tmp_ordered.end(),
              [](const WordFreq& x, const WordFreq& y) {
                return x.second > y.second;
              });
    int pair_count = 0;
    for (auto it: tmp_ordered) {
      pair_count ++;
      ordered.push_back(it);
      if (pair_count > vocab_size_in) {
        break;
      }
    }
